...
def rle_encod(text='text_words.txt', code_text='text_code_words.text'):
    if path.exists(text) and not path.exists(code_text):
        with open(text) as my_f_1, \
                open(code_text) as my_f_2:
            for i in my_f_1:
...
rle_decode(input('Enter the name of the file with the text: '), input('Enter the file name to record: '))
